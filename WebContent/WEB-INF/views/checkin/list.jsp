...
<div id="add-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'" style="width:450px; padding:10px;">
    ...
    <table>
        <tr>
            <td>
                <div>
                    <video autoplay playsinline id="player" style="width: 128px; height: 72px"></video>
                </div>
            </td>
            <td>
                <div>
    ...
    <form id="add-form" method="post">
        <table>
            ...
            <tr>
                ...
                <td><textarea id="add-remark" name="remark" rows="2" class="wu-textarea" style="width:260px"></textarea></td>
            	<td></td>
            </tr>
        </table>
    </form>
...
<!-- 人脸对比窗口 -->
<div id="compare-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'" style="width:450px; padding:10px;">
    <form id="compare-form" method="post">
        <table>
            <tr>
                <p style="color: red">照片路径中的“\”需改为“\\”</p>
            </tr>
            <tr>
                <td align="right">当前照片路径:</td>
                <td><input type="text" id="compare-photo1" name="photo1" class="wu-text easyui-validatebox" data-options="required:true, missingMessage:'请输入当前照片路径'"  /></td>
                <td></td>
            </tr>
            <tr>
                <td align="right">预留照片路径:</td>
                <td><input type="text" id="compare-photo2" name="photo2" class="wu-text easyui-validatebox" data-options="required:true, missingMessage:'请输入预留照片路径'" /></td>
                <td></td>
            </tr>
            <%--<tr>
                <td align="right">顾客:</td>
                <td>
                    <select id="compare-accountId" name="accountId" class="easyui-combobox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择入住顾客'">
            ...
            <tr>
                <div>
                    <img id="account-picture" src="" alt="" style="width: 128px; height: 72px"/>
                </div>
            </tr>
            <tr>
                <div>
                    <canvas id="account-picture1"></canvas>
                </div>
            </tr>--%>
        </table>
    </form>
...
<script type="text/javascript">
    ...
    function downLoad(url) {
        ...
        oA.download = 'photo';// 设置下载的文件名，默认是'photo'
        oA.href = url;
	...
	*/
	...
	function compare() {
        let validate = $("#compare-form").form("validate");
        if(!validate){
            $.messager.alert("消息提醒","请检查你输入的数据!","warning");
            return false;
        }
        let data = $("#compare-form").serialize();
        $.ajax({
            url:'comparePhoto',
            dataType:'json',
            type:'post',
            ...
            success:function(data){
                if(data.type === "success"){
                    $.messager.alert('信息提示',data.msg+"相似度为"+data.result,'info');
	...
	*/
	...
	$("#search-btn").click(function(){
		...
		var room = $("#search-room").combobox('getValue');
    ...
    //人脸识别，选择顾客后显示图片
    $("#compare-accountId").combobox({
        onSelect:function(data){
            //todo 显示图片
            document.getElementById("account-picture").src=data.value.photo;
            let context = document.getElementById("account-picture1").getContext('2d');
            let img = new Image();
            img.src = data.value.photo;
	...
	*/
	$('#data-datagrid').datagrid({
		...
		columns:[[
			...
			{ field:'createTime',title:'预订时间',width:150,formatter:function(value,row,index){
				return format(value);
			}},
		...
		$('#show-order-dialog').dialog({
            ...
            buttons: [{
                ...
                handler: function(){
            		...
            		$("#add-checkinPrice").val(selectedOrder.checkinPrice);
                    ...
                    $("#edit-checkinPrice").val(selectedOrder.checkinPrice);
            ...
            onBeforeOpen:function(){
            	$('#order-datagrid').datagrid({
            		...
            		columns:[[
            			...
            			{ field:'name',title:'预订人',width:60,sortable:true},
            			{ field:'arriveDate',title:'入住日期',width:100,sortable:true},
            			{ field:'leaveDate',title:'离店日期',width:100,sortable:true},
            			{ field:'status',title:'状态',width:100,formatter:function(value,row,index){
            				switch(value){
