...
public class RoomTypeController {
    ...
    public Map<String, String> add(RoomType roomType){
        ...
        roomType.setAvailableNum(roomType.getRoomNum());//默认房间数等于可用房间数
        roomType.setBookNum(0);//设置预订数0
    ...
    public Map<String, String> edit(RoomType roomType){
        ...
        roomType.setAvailableNum(existRoomType.getAvailableNum() + offset);
        if(roomType.getAvailableNum() <= 0){
            roomType.setAvailableNum(0);//没有可用房间
            roomType.setStatus(0);//房型已满
            if(roomType.getAvailableNum() + existRoomType.getLivedNum() + existRoomType.getBookNum() > roomType.getRoomNum()){
                ret.put("type", "error");
